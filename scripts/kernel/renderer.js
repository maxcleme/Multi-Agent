...
var Renderer = function (scale) {
  ...
  this.init = function () {
    ...
    if (!this.scale) {
      var scaleHeight = (window.innerHeight / 2) / this.environment.options.height;
      var scaleWidth = (window.innerWidth / 2) / this.environment.options.width;
      this.scale = Math.floor(Math.min(scaleHeight, scaleWidth));
    }

    var canvas = document.createElement("canvas");
    canvas.setAttribute("width", "" + (this.environment.options.width * this.scale));
    canvas.setAttribute("height", "" + ((this.environment.options.height+1) * this.scale));
  ...
  this.displayDijkstraNumbering = function(){
    ...
    for(var x = 0; x < dijkstraGrid.length; x++){
      for(var y = 0; y < dijkstraGrid[x].length; y++){
        ...
        this.context.fillText(""+dijkstraGrid[x][y], (x)* this.scale + this.scale/3, ((y+1)* this.scale - this.scale/3));
      }
    }
  };
   ...
   */
  this.clean = function () {
    this.context.clearRect(0, 0, this.environment.options.width * this.scale, this.environment.options.height * this.scale);
    this.context.fillStyle = "black";
    this.context.fillRect(0, 0, this.environment.options.width * this.scale, this.environment.options.height * this.scale);
  };
